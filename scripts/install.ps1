...
<#
.SYNOPSIS
    zabadev — Senior ZaBaDeV Installer for Windows
    One command to install Senior ZaBaDeV on OpenCode.

.DESCRIPTION
    Downloads and installs the zabadev binary for Windows.
    Installs Senior ZaBaDeV directly on OpenCode with all components.
...
.EXAMPLE
    ...
    # Or download and run:
    Invoke-WebRequest -Uri https://raw.githubusercontent.com/zabadev/agent-ai/main/scripts/install.ps1 -OutFile install.ps1
    .\install.ps1
...
$GITHUB_OWNER = "zabadev"
$GITHUB_REPO = "agent-ai"
$BINARY_NAME = "zabadev"
...
function Show-Banner {
    ...
    Write-Host ""
    Write-Host "  Senior ZaBaDeV Installer — One command, fully configured" -ForegroundColor DarkGray
    Write-Host ""
}
...
function Get-LatestVersion {
    ...
    try {
        $response = Invoke-RestMethod -Uri $url -Headers @{ "User-Agent" = "zabadev-installer" }
    } catch {
...
function Install-ViaBinary {
    ...
    $tmpDir = Join-Path $env:TEMP "zabadev-install-$(Get-Random)"
    ...
    try {
        ...
        if (-not $installDir) {
            $installDir = Join-Path $env:LOCALAPPDATA "zabadev\bin"
        }
...
function Test-Installation {
    ...
    $locations = @(
        (Join-Path $env:LOCALAPPDATA "zabadev\bin\$BINARY_NAME.exe"),
...
function Show-NextSteps {
    ...
    Write-Host ""
    Write-Host "Senior ZaBaDeV is now installed on OpenCode with:" -ForegroundColor White
    Write-Host "  • Engram (persistent memory)" -ForegroundColor Cyan
    Write-Host "  • SDD workflow (Spec-Driven Development)" -ForegroundColor Cyan
    Write-Host "  • Full skill set" -ForegroundColor Cyan
    Write-Host "  • Context7 MCP server" -ForegroundColor Cyan
    Write-Host "  • GGA (Global Agent)" -ForegroundColor Cyan
    Write-Host "  • Security-first permissions" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "Run 'zabadev' to install/update or 'zabadev --help' for options" -ForegroundColor DarkGray
    Write-Host ""
}
