...
End-to-end tests that validate the `zabadev` installer binary inside Docker containers running real Linux distributions.
...
docker build -f e2e/Dockerfile.ubuntu -t zabadev-e2e-ubuntu .
docker run --rm zabadev-e2e-ubuntu

# Run with full E2E on Arch
docker build -f e2e/Dockerfile.arch -t zabadev-e2e-arch .
docker run --rm -e RUN_FULL_E2E=1 zabadev-e2e-arch

# Interactive debugging
docker run --rm -it zabadev-e2e-ubuntu /bin/bash
```
...
4. Build the `zabadev` binary for `linux/amd64`
