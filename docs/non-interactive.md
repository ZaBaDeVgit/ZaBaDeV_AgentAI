...
```bash
go run ./cmd/zabadev install [flags]
```
...
- `--persona`: explicit persona id (`senior-zabadev`, `neutral`, `custom`). Legacy `gentleman` is accepted as an alias and maps to `senior-zabadev`.
