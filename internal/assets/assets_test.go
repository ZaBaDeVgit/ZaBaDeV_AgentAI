...
func TestAllEmbeddedAssetsAreReadable(t *testing.T) {
	expectedFiles := []string{
		...
		"opencode/persona-senior-zabadev.md",
		...
		"generic/persona-senior-zabadev.md",
...
func TestOpenCodeEmbeddedAssetLayout(t *testing.T) {
	...
	for _, name := range []string{"commands", "plugins", "persona-gentleman.md", "persona-senior-zabadev.md", "sdd-overlay-single.json", "sdd-overlay-multi.json"} {
		if !seen[name] {
...
func TestEmbeddedAssetCount(t *testing.T) {
	...
	// We expect 25 skill directories (10 SDD + 12 framework/testing + 3 foundation).
	if skillDirs != 25 {
