...
func TestInjectClaudeGentlemanWritesOutputStyleFile(t *testing.T) {
	...
	text := string(content)
	if !strings.Contains(text, "name: ZaBaDeV") {
		t.Fatal("Output style file missing YAML frontmatter 'name: ZaBaDeV'")
	}
	if !strings.Contains(text, "keep-coding-instructions: true") {
		t.Fatal("Output style file missing 'keep-coding-instructions: true'")
	}
	if !strings.Contains(text, "ZaBaDeV Output Style") {
		t.Fatal("Output style file missing 'ZaBaDeV Output Style' heading")
	}
}
...
func TestInjectClaudeGentlemanMergesOutputStyleIntoSettings(t *testing.T) {
	...
	if outputStyle != "ZaBaDeV" {
		t.Fatalf("settings.json outputStyle = %q, want %q", outputStyle, "ZaBaDeV")
	}
...
func TestInjectOpenCodeSeniorWritesAgentsFile(t *testing.T) {
	home := t.TempDir()

	result, err := Inject(home, opencodeAdapter(), model.PersonaSeniorZaBaDeV)
	if err != nil {
		t.Fatalf("Inject() error = %v", err)
	}
	if !result.Changed {
	...
	text := string(content)
	if !strings.Contains(text, "Senior ZaBaDeV") {
		t.Fatal("AGENTS.md missing Senior ZaBaDeV prompt content")
	}
}

func TestInjectOpenCodeDoesNotWriteLegacyPromptFiles(t *testing.T) {
	home := t.TempDir()
	...
	for _, legacy := range []string{"SENIOR-ZABADEV.md", "GENTLEMAN.md"} {
		path := filepath.Join(home, ".config", "opencode", legacy)
		if _, err := os.Stat(path); !os.IsNotExist(err) {
			t.Fatalf("legacy prompt file %q should not be created", legacy)
		}
	}
}

func TestInjectOpenCodeWritesSingleSeniorAgentOverlay(t *testing.T) {
	home := t.TempDir()
	...
	agentMap, ok := settings["agent"].(map[string]any)
	if !ok {
		t.Fatalf("agent map missing or invalid: %#v", settings["agent"])
	}

	if len(agentMap) != 1 {
		t.Fatalf("agent map should expose exactly one public agent, got %d entries", len(agentMap))
	}

	seniorRaw, ok := agentMap[openCodeSeniorAgentID]
	if !ok {
		t.Fatal("opencode.json missing senior-zabadev agent")
	}

	seniorAgent, ok := seniorRaw.(map[string]any)
	if !ok {
		t.Fatalf("senior-zabadev has unexpected type: %T", seniorRaw)
	}

	if seniorAgent["mode"] != "primary" {
		t.Fatalf("senior-zabadev mode = %#v, want %q", seniorAgent["mode"], "primary")
	}

	if seniorAgent["prompt"] != "{file:./AGENTS.md}" {
		t.Fatalf("senior-zabadev prompt = %#v, want %q", seniorAgent["prompt"], "{file:./AGENTS.md}")
	}

	tools, ok := seniorAgent["tools"].(map[string]any)
	if !ok {
		t.Fatalf("senior-zabadev tools missing or invalid: %#v", seniorAgent["tools"])
	}

	for _, toolName := range []string{"read", "write", "edit", "bash", "delegate", "delegation_read", "delegation_list"} {
		if val, ok := tools[toolName].(bool); !ok || !val {
			t.Fatalf("senior-zabadev missing enabled tool %q", toolName)
		}
	}
}

func TestInjectOpenCodeSeniorZaBaDeVStaysSinglePrimaryAgent(t *testing.T) {
	home := t.TempDir()
	...
	if !strings.Contains(string(agentsContent), "Senior ZaBaDeV") {
		t.Fatal("AGENTS.md should contain the Senior ZaBaDeV prompt when senior persona is selected")
	}
	...
	seniorAgent := agentMap["senior-zabadev"].(map[string]any)
	if seniorAgent["mode"] != "primary" {
		t.Fatalf("senior-zabadev mode = %#v, want %q", seniorAgent["mode"], "primary")
	}
	if seniorAgent["prompt"] != "{file:./AGENTS.md}" {
	...
	if _, exists := agentMap["gentleman"]; exists {
		t.Fatal("opencode.json should not expose the legacy gentleman agent")
	}
	if _, exists := agentMap["sdd-orchestrator"]; exists {
		t.Fatal("opencode.json should not expose the legacy sdd-orchestrator agent")
	}
}

func TestInjectClaudeSeniorZaBaDeVWritesSeniorPromptWithoutOutputStyle(t *testing.T) {
	home := t.TempDir()

	result, err := Inject(home, claudeAdapter(), model.PersonaSeniorZaBaDeV)
	if err != nil {
		t.Fatalf("Inject() error = %v", err)
	}
	if !result.Changed {
	...
	text := string(content)
	if !strings.Contains(text, "Senior ZaBaDeV") {
		t.Fatal("CLAUDE.md missing Senior ZaBaDeV prompt content")
	}
	if strings.Contains(text, "ZaBaDeV Output Style") {
		t.Fatal("Senior ZaBaDeV persona should not inject the gentleman output style")
	}

	stylePath := filepath.Join(home, ".claude", "output-styles", "gentleman.md")
	if _, err := os.Stat(stylePath); !os.IsNotExist(err) {
		t.Fatal("Senior ZaBaDeV persona should not write gentleman output-style file")
	}
}
