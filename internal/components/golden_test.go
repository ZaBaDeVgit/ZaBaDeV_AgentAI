...
import (
	...
	codexagent "github.com/zabadev/agent-ai/internal/agents/codex"
	"github.com/zabadev/agent-ai/internal/agents/cursor"
...
func TestGoldenPersona_OpenCode_SeniorZaBaDeV(t *testing.T) {
	home := t.TempDir()

	result, err := persona.Inject(home, opencodeAdapter(), model.PersonaSeniorZaBaDeV)
	if err != nil {
		t.Fatalf("persona.Inject(opencode, senior-zabadev) error = %v", err)
	}
	if !result.Changed {
		t.Fatalf("persona.Inject(opencode, senior-zabadev) changed = false")
	}

	agentsMD := readTestFile(t, filepath.Join(home, ".config", "opencode", "AGENTS.md"))
	assertGolden(t, "persona-opencode-senior-zabadev.golden", agentsMD)
}
