...
func TestInjectOpenCodeMigratesLegacyAgentsKey(t *testing.T) {
  ...
  "agents": {
	...
	if _, ok := agentMap["senior-zabadev"]; !ok {
		t.Fatal("senior-zabadev agent missing after merge")
	}
}
...
func TestInjectOpenCodeMultiMode(t *testing.T) {
	...
	// Multi overlay must contain the primary senior agent + 9 sub-agents = 10 agents.
	if len(agentMap) != 10 {
		t.Fatalf("agent count = %d, want 10", len(agentMap))
	}

	// Verify the primary senior agent is present.
	orchestratorRaw, ok := agentMap["senior-zabadev"]
	if !ok {
		t.Fatal("missing senior-zabadev agent")
	}
	orchestratorAgent, ok := orchestratorRaw.(map[string]any)
	if !ok {
		t.Fatalf("senior-zabadev has unexpected type: %T", orchestratorRaw)
	}
...
func TestInjectOpenCodeEmptySDDModeDefaultsSingle(t *testing.T) {
	...
	// Empty mode defaults to single — only senior-zabadev, no sub-agents.
	if _, ok := agentMap["senior-zabadev"]; !ok {
		t.Fatal("missing senior-zabadev agent")
	}
...
func TestInjectOpenCodeSingleToMultiSwitch(t *testing.T) {
	...
	if _, ok := agentMap["senior-zabadev"]; !ok {
		t.Fatal("missing senior-zabadev after switch to multi")
	}
	if _, ok := agentMap["sdd-apply"]; !ok {
...
func TestInjectOpenCodeMultiModeWithPreExistingMinimalConfig(t *testing.T) {
	...
	if _, ok := agentMap["senior-zabadev"]; !ok {
		t.Fatal("missing senior-zabadev after merge with pre-existing config")
	}
	if _, ok := agentMap["sdd-apply"]; !ok {
...
`
	...
	for _, agentName := range []string{
		"senior-zabadev", "sdd-init", "sdd-explore", "sdd-propose",
