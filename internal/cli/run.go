...
import (
	...
	"github.com/zabadev/agent-ai/internal/components/engram"
	"github.com/zabadev/agent-ai/internal/components/gga"
	"github.com/zabadev/agent-ai/internal/components/mcp"
	"github.com/zabadev/agent-ai/internal/components/permissions"
	"github.com/zabadev/agent-ai/internal/components/persona"
	"github.com/zabadev/agent-ai/internal/components/sdd"
	"github.com/zabadev/agent-ai/internal/components/skills"
	"github.com/zabadev/agent-ai/internal/components/theme"
...
func RunInstall(args []string, detection system.DetectionResult) (InstallResult, error) {
	...
	// Allow partial success for non-critical component failures (e.g., GGA already installed).
	// Only fail if there are no successful steps at all.
	if result.Execution.Err != nil && !hasAnySuccessfulStep(result.Execution) {
...
// hasAnySuccessfulStep checks if any step in the execution result was successful.
// This allows partial success for non-critical component failures.
func hasAnySuccessfulStep(execResult pipeline.ExecutionResult) bool {
	stages := []pipeline.StageResult{execResult.Prepare, execResult.Apply, execResult.Rollback}
	for _, stage := range stages {
		for _, step := range stage.Steps {
			if step.Status == pipeline.StepStatusSucceeded {
				return true
			}
		}
	}
