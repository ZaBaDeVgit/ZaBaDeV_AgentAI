# PRD: ZaBaDeV Installer

> **One command. Any agent. Any OS. The ZaBaDeV ecosystem — configured and ready.**
...
**This installer eliminates that gap entirely.** You pick your agent(s), you pick your config level, and the entire ZaBaDeV ecosystem gets injected into your tools — ready to go. From zero to championship-level AI development in minutes.

---

## 2. Vision

**The ZaBaDeV ecosystem — installable by anyone, on any agent, on any OS, in one command.**
...
### 6.4 GGA — ZaBaDeV Guardian Angel (AI Code Review)
...
#### Persona Selection — "Your own ZaBaDeV!"

The ZaBaDeV persona is the heart of this ecosystem, but it's **100% optional**. The user chooses their experience:

| Persona Option | Description | What it Configures |
|---------------|-------------|-------------------|
| **ZaBaDeV Mode** | "Your own ZaBaDeV!" — The Senior Architect mentor who teaches, challenges, and pushes you to understand concepts before code. Castellano de Jaén for Spanish input, direct English otherwise. Uses Tony Stark/Jarvis analogies. | Full persona in CLAUDE.md / opencode agents / .cursorrules, custom thinking verbs, teaching-first behavior |
...
|---------------|---------------------|
| Theme | ZaBaDeV dark theme (navy/steel/gold) or default |
...
| Thinking verbs | Custom spinner text — Jaén phrases like "Tomando un Cafecito mientras Pienso" (only with ZaBaDeV persona) |
...
- R-CONFIG-02: Selecting "ZaBaDeV Mode" MUST display the tagline "Your own ZaBaDeV!" and a brief preview of how the agent will behave
...
- R-CONFIG-07: Thinking verbs and Jaén expressions MUST only be configured when ZaBaDeV persona is selected
...
curl -sL get.gentleman.ai/ai | sh
     ...
     │  Choose your Persona             │
     │                                  │
     │  ★ "Your own ZaBaDeV!"           │  ← Senior Architect mentor, teaches,
      │     The mentor who pushes you     │     challenges, castellano de Jaén
     ...
     │                                  │
     │  ★ Full ZaBaDeV                  │  ← Everything: Engram + SDD + Skills
...
| Persona Selection | "Your own ZaBaDeV!" / Neutral / Custom |
| Preset Selection | Full ZaBaDeV / Ecosystem Only / Minimal / Custom |
...
flowchart TD
    ...
    subgraph PHASE_3["Phase 3: Backup"]
        BACKUP[Backup existing configs<br/>~/.zabadev-backup-TIMESTAMP/]
    end
...
|--------|----------------|---------|-------------|
| `full-gentleman` | All agents detected + Engram + SDD + all skills + MCP + theme | "Your own ZaBaDeV!" | The complete experience. Everything configured, ZaBaDeV persona, dark theme, the works. |
...
| Homebrew | `brew install Gentleman-Programming/tap/zabadev` | P0 |
| Go install | `go install github.com/Gentleman-Programming/zabadev/cmd/zabadev@latest` | P1 |
| Direct binary | Download from GitHub Releases | P1 |
| winget (Windows) | `winget install zabadev` | P2 |
...
**Requirements:**
- R-UPDATE-01: The installer MUST support `zabadev update` to check for and install newer versions of itself
- R-UPDATE-02: The installer MUST support `zabadev update --skills` to pull latest skill versions for all configured agents
- R-UPDATE-03: The installer MUST support `zabadev update --engram` to update Engram to the latest version
...
When the installer completes with "Full ZaBaDeV" preset + Claude Code + OpenCode:

**Claude Code:**
- `~/.claude/CLAUDE.md` — ZaBaDeV persona with SDD orchestrator
- `~/.claude/settings.json` — Security-first permissions, ZaBaDeV theme, vim mode, custom statusline, thinking verbs
...
**OpenCode:**
- `~/.config/opencode/opencode.json` — Agent (`senior-zabadev`) + hidden `sdd-*` sub-agents in multi mode, MCP servers (engram, context7), Engram plugin, ZaBaDeV theme
...
- R-REL-03: The installer MUST support `zabadev repair` to re-run failed steps
...
| Aspect | Gentleman.Dots | ZaBaDeV Installer |
|--------|---------------|----------------------|
...
| Can use together | Yes — install Gentleman.Dots first for dev env, then ZaBaDeV for the AI layer | Same |
...
1. **Naming**: `zabadev`, `zabadev`, `gai`, or something else? Should it be part of the `Gentleman-Programming` org or standalone?
...
zabadev install --preset gentleman --agents claude-code,opencode

# Minimal setup, just Claude Code with basic security
zabadev install --preset minimal --agents claude-code

# Team provisioning from shared profile
zabadev install --profile ./team-ai-config.yaml

# Update all skills to latest
zabadev update --skills

# Update Engram
zabadev update --engram
...
zabadev restore --id 2026-02-27-143022
